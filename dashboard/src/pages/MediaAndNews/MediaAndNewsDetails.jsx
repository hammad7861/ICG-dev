...
import { getMediaAndNewss } from "../../services/api/mediaAndNews";

const MediaAndNewsDetails = () => {
  const columns = [
    {
      title: "Title",
      dataIndex: "title",
    ...
    },
    {
      ...
      render: ({ _id }) => {
        return (
          <div
            onClick={() => {
              navigate(`/mediaAndNews-details/${_id}`);
            }}
  ...
  const { data, isPending, isFetching } = useQuery({
    queryKey: ["get-mediaAndNewss", page, limit],
    queryFn: () => getMediaAndNewss(page, limit),
  });
  return (
      ...
      <div className="mt-[25px] flex items-center justify-between">
        ...
        <div className="">
          ...
          <Button
            className="btn-green"
            onClick={() => {
              navigate("/mediaAndNews-details/add-mediaAndNews");
            }}
          >
            <Plus /> Add Media and News
          </Button>
        </div>
      </div>
      <div className="w-full mt-[25px]">
        <AntdTable
          dataSource={data?.mediaAndNews ?? []}
          columns={columns}
          setPage={setPage}
          page={page}
          limit={limit}
...
export default MediaAndNewsDetails;
